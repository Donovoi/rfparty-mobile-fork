...
module.exports = class BleAdvDocument extends Dataparty.IDocument {
  ...
  parsePacket() {
    ...
    this.data.packet.parsed = {
      ...
      gapFields,
      hasUnknownService:
        uuids.length > 0 ? uuidSearch.unknown.length > 0 : undefined,
  ...
  static async indexBleDevice(party, dev, point) {
    console.log(
      "indexBleDevice called for:",
      dev?.id,
      "with location:",
      JSON.stringify(point)
    );
    // Validate advertising data exists before processing
    // dev.advertising is an ArrayBuffer after conversion by the BLE plugin
    if (
      !dev ||
      !dev.advertising ||
      !(dev.advertising instanceof ArrayBuffer) ||
      dev.advertising.byteLength === 0
    ) {
      console.log(
        "indexBleDevice - SKIPPING device - advertising check failed:",
        dev?.id,
        "advertising type:",
      ...
      debug(
        "indexBleDevice - skipping device with missing advertising data:",
        dev?.id
      );
      return null;
    }
    ...
    console.log(
      "indexBleDevice - processing device:",
      dev.id,
      "advertising bytes:",
      dev.advertising.byteLength
    );
    // Convert ArrayBuffer to base64 string for storage and hashing
    const uint8Array = new Uint8Array(dev.advertising);
    const binaryString = String.fromCharCode.apply(null, uint8Array);
    const base64Data = btoa(binaryString);

    const now = moment().valueOf();
    const packetHash = md5(base64Data);

    let advs = await party
      .find()
      .type("ble_adv")
      .where("address")
      .equals(dev.id.toLowerCase())
      .where("packet.hash")
      .equals(packetHash)
      .limit(1)
      .exec();
    ...
    if (!bleAdvDoc) {
      bleAdvDoc = await BleAdvDocument.createFromBleDevice(
        party,
        dev,
        packetHash,
        point,
    ...
    bleAdvDoc.data.geobounds = GeoUtils.updatGeoBoundsByPoint(
      bleAdvDoc.data.geobounds,
      point
    );
    bleAdvDoc.data.location = GeoUtils.updateLocationBounds(
      bleAdvDoc.data.location,
      point
    );
    bleAdvDoc.data.timebounds = GeoUtils.updateTimebounds(
      bleAdvDoc.data.timebounds,
      now
    );
    bleAdvDoc.data.best = GeoUtils.updateBestRssi(
      bleAdvDoc.data.best,
      currentRssi
    );
    bleAdvDoc.data.worst = GeoUtils.updateWorstRssi(
      bleAdvDoc.data.worst,
      currentRssi
    );
  ...
  static async createFromBleDevice(party, dev, packetHash, point, base64Data) {
    debug("create");
